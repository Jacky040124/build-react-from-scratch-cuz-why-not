...
test('creates simple element', () => {
  const element = window.createElement("div", null, "Hello")
...
test('creates element with props', () => {
  const element = window.createElement("div", { id: "test" }, "Hello")
  expect(element.type).toBe("div")
...
test('creates element with multiple children', () => {
  const element = window.createElement("div", null, "Hello", "World")
  expect(element.type).toBe("div")
  expect(element.props.children[0].type).toBe("TEXT_ELEMENT")
  expect(element.props.children[0].props.nodeValue).toBe("Hello")
...
test('creates nested elements', () => {
  const child = window.createElement("span", null, "Child")
  const parent = window.createElement("div", null, child)
